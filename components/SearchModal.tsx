...
import { searchCoins } from '@/lib/coingecko.client';
...
export const SearchModal = ({
    ...
    // fetch trending coins from our API when modal is open and there's no query
    const { data: trendingApiData, isValidating: isTrendingApi } = useSWR(
        open && !debouncedQuery ? '/api/trending' : null,
        async () => {
    ...
    const runtimeTrendingCoins: TrendingCoin[] = (
        initialTrendingCoins && initialTrendingCoins.length > 0
            ? initialTrendingCoins
            : trendingApiData?.coins || []
    ).slice(0, TRENDING_LIMIT);

    const trendingCoins = runtimeTrendingCoins;
